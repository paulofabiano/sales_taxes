...
RSpec.describe SalesTaxes::TaxCalculator do
  ...
  context "exempt goods (book, food, medical) with no import duty" do
  ...
  context "non-exempt, domestic good" do
  ...
  context "exempt but imported good" do
  ...
  context "non-exempt and imported good" do
  ...
  context "rounding edge case" do
    it "rounds 1.13 to 1.15 after tax calculation" do
      dummy = product(price: "11.30", category: :other)
      expect(calculator.tax_for(dummy)).to eq(BigDecimal("1.15"))
    end
  end

  context "with an empty rule set injected" do
